...
유사한 맥락에서, KR 100930208B1은 프로젝트 생산성 관리를 위해 작업별 투입 시간과 산출물을 측정하는 시스템을 제시하였으나, 작업 식별자에 공정 단계(Stage)나 업무 영역(Area) 등의 구조 정보가 인코딩되지 않아 식별자만으로는 작업의 위치나 맥락을 파악할 수 없다. 또한 KR 101150014B1은 온라인 캘린더를 활용하여 프로젝트 시간을 할당하고 자동 조정하는 기술을 제안하였으나, 시간 할당 중심의 정적 관리에 그치며 작업 간 의존성이나 다차원 구조를 식별자 수준에서 표현하지 못하는 한계를 갖는다.
...
【특허문헌】
(미국)
· US 9,552,229 B2: 작업 스케줄링을 위한 시스템 및 방법 (Atlassian)
· US 11,449,836 B1: 작업 단위와의 사용자 참여 시스템 (Asana)
· US 10,140,151 B2: DAG 정보를 활용한 작업 그룹화
· US 8,245,153 B2: 간트 차트 표시를 위한 시스템 및 방법

(한국)
· KR 10-0935304 B1: 프로젝트 진척관리 시스템 및 그 방법
· KR 100930208B1: 프로젝트 생산성 관리 시스템 및 그 방법
· KR 101150014B1: 온라인 캘린더링 시스템 내의 프로젝트 및 회의의 시간할당을 위한 시간 관리 표현 및 자동화
...
첫째, Apache Airflow(US 10,140,151 B2) 등 워크플로우 오케스트레이션 시스템과의 차별성이다. Apache Airflow는 사용자가 파이썬(Python) 코드를 사용하여 DAG(Directed Acyclic Graph) 구조를 명시적으로 작성해야 하며, 이는 각 Task 간의 의존 관계를 `task_a >> task_b` 형태로 코딩하는 프로그래밍 기반 접근 방식이다. 사용자는 복잡한 워크플로우를 표현하기 위해 반복문, 조건문, 함수 정의 등 고급 프로그래밍 지식을 필요로 하며, 의존성 구조 변경 시 코드를 직접 수정하고 디버깅해야 한다.

Apache Airflow의 기술 분야에서 통상의 기술자들은 워크플로우 의존성을 명시적 코드로 선언하는 것을 당연한 설계 원칙으로 여겨왔다. Apache Airflow의 공식 문서와 주요 논문들(Cruz, R. et al., 2021; Krishna, S. et al., 2020)은 모두 "의존성은 코드로 명시해야 한다(dependencies must be explicitly coded)"는 전제에서 출발하며, 식별자에 구조 정보를 인코딩하여 자동 추론한다는 발상은 해당 기술 분야에서 제안된 바 없다. 이는 통상의 기술자가 기존 기술 사상의 연장선에서는 쉽게 도달할 수 없는 발상의 전환이다.

반면, 본 발명의 SAL ID 기반 시스템은 복잡한 코드 작성 없이 식별자 자체에 3차원 좌표(Stage, Area, Level)가 인코딩되어 있어, 파서(40)가 정규식 파싱만으로 의존성 구조를 자동으로 추론하고 구축한다. 본 발명자는 "의존성은 명시적으로 선언되어야 한다"는 종래 기술의 통념을 깨고, "식별자 자체가 구조 정보를 내재하면 명시적 선언 없이 자동 추론할 수 있다"는 역발상을 처음으로 제시하였다. 예를 들어, `S2A1L1`이라는 SAL ID는 파싱 즉시 "Stage 2, Area A1, Level 1"로 분해되어, 동일 Stage와 Area 내에서 Level 값이 작은 Task가 자동으로 선행 작업임을 시스템이 인지한다. 이는 Apache Airflow에서 `task_a >> task_b`로 명시적 코딩이 필요한 부분을 SAL ID의 구조적 파싱만으로 대체하는 것으로, No-Code 방식의 의존성 자동 구축이라는 예상치 못한 효과를 발생시킨다.

또한, 본 발명의 시퀀서(53)는 Level 충돌을 자동 감지하여 SAL ID를 동적으로 조정함으로써, Apache Airflow에서 수동으로 수행해야 하는 DAG 재작성 과정을 완전 자동화한다. Apache Airflow의 식별자는 `task_id`라는 단순 문자열로서 구조 정보를 포함하지 않으나, 본 발명의 SAL ID는 식별자 자체가 3차원 좌표계의 위치 정보를 내재하여 별도의 매핑 테이블 조회 없이 즉시 작업의 위치와 맥락을 파악할 수 있다.

둘째, Jira, Asana 등 프로젝트 관리 도구(US 9,552,229 B2, US 11,449,836 B1), 생산성 관리 시스템(KR 100930208B1), 온라인 캘린더 기반 프로젝트 시간 할당 시스템(KR 101150014B1)과의 차별성이다. 이들 시스템은 공통적으로 식별자에 구조 정보가 부재하고 시간 할당 중심의 정적 관리에 그친다는 한계를 갖는다.

Jira, Asana의 식별자(예: PROJECT-1234, ISSUE-5678)는 데이터베이스의 자동 증가(Auto-increment) 필드로 생성되는 순차적 참조 번호이며, SK C&C의 생산성 관리 시스템(KR 100930208B1)은 소프트웨어 개발 프로젝트에서 개발자별 생산성을 분석·설계·개발 단계별로 관리하고 결함 추적 서버를 통해 품질을 관리하지만, 작업 식별자 자체에 단계(Stage)나 영역(Area) 정보가 인코딩되어 있지 않아 작업 간 의존성을 식별자만으로 추론할 수 없다. Microsoft의 온라인 캘린더 시스템(KR 101150014B1)은 프로젝트와 회의에 자동으로 시간을 할당하고 우선순위에 따라 일정을 재조정하는 기능을 제공하나, 역시 작업 식별자가 단순 참조 번호에 그쳐 식별자만으로는 해당 작업이 어느 단계(Stage)와 영역(Area)에 속하는지 즉시 파악할 수 없다.

반면, 본 발명의 SAL ID는 식별자 자체가 `S{stage}{area}{level}{variant?}` 형식으로 구성되어, 예를 들어 `S3BA2`는 "Stage 3의 Backend APIs 영역, Level 2 작업"임을 즉시 알 수 있다. KR 100930208B1이 개발자별 생산성을 사후적으로 분석하는 것과 달리, 본 발명은 SAL ID의 Level 값을 통해 작업 간 선후 관계를 식별자 생성 시점부터 사전적으로 정의하며, KR 101150014B1이 시간 할당에 중점을 두는 것과 달리, 본 발명은 3차원 좌표계 내에서 작업의 논리적 위치(Stage-Area-Level)를 정의하여 의존성 구조를 자동으로 구축한다. 또한, Jira 등에서 작업의 속성이 변경되면 기존 Issue 번호가 그대로 유지되어 변경 전후 맥락이 혼재되거나, 또는 새 Issue를 생성하면서 기존 Issue와의 연결이 끊어지는 참조 무결성 문제가 발생한다. 본 발명은 SAL ID 변경 시 기존 ID를 삭제하지 않고 새로운 ID를 체인 방식으로 연결하는 Append-Only 구조를 채택하여, 구(Old) SAL ID로 조회해도 리졸버(74)가 체인을 추적하여 최신 SAL ID를 자동 반환함으로써 참조 무결성을 자가 치유(Self-Healing)한다.

셋째, 간트 차트 시스템(US 8,245,153 B2) 및 3차원 간트 차트 연구(ResearchGate, 2019)와의 차별성이다. 기존 간트 차트 특허는 시간축(X)과 작업명(Y)의 2차원 평면에 작업을 배치하는 시각화 방법에 한정되며, 3차원 간트 차트 연구 역시 X축을 시간, Y축을 프로젝트 번호, Z축을 자원으로 설정한 시각화(Visualization) 방법을 제안하였으나 개념적 제안에 그친다. 식별자 자체에 좌표 정보가 인코딩되어 있지 않고, 파싱을 통한 자동 의존성 추론, Task 지시서 자동 생성, 검증 자동화, ID 체인 관리 등 시스템적 오케스트레이션 메커니즘이 부재하다. 본 발명은 단순 시각화를 넘어서, SAL ID를 파싱하여 자동으로 Task 지시서 및 검증 지시서를 생성하는 SAL 그리드 생성기(52), Level 충돌을 감지하여 SAL ID를 동적 조정하는 시퀀서(53), 에이전트와 도구를 자동 배정하는 배분기(54), Task 실행 및 검증 결과를 실시간 기록하는 SAL 그리드 레코더(60), ID 변경 이력을 체인으로 관리하는 ID 체인 관리 모듈(70), 그리고 3차원 좌표(Stage-X축, Area-Y축, Level-Z축)로 매핑하여 실시간 프로젝트 진행 현황을 시각화하는 SAL 그리드 뷰어(60)를 포함하는 완전한 통합 오케스트레이션 시스템을 구성한다. 또한, 본 발명은 Stage별로 전체 Task의 완료 여부, 빌드 및 테스트 통과 여부, 의존성 체인 완결성을 검증하는 자동 검증 리포트 생성 및 Stage Gate 승인 프로세스를 포함하며, 승인이 거절될 경우 자동으로 Task 실행 단계로 회귀하는 피드백 루프를 내장하여 실행-검증-기록-승인의 순환적 오케스트레이션 사이클을 완성한다.
...
3.5 SAL ID의 구조적 특성
본 발명의 SAL ID는 절차적 순서, 실행적 선후 관계의 의존성, 병렬성 및 인접성 정보를 구조적으로 내재하고 있다. 이는 SAL ID의 형식 자체가 다음과 같은 정보를 암묵적으로 인코딩하고 있음을 의미한다.

(1) 절차적 순서 (Procedural Order)
SAL ID의 Stage 값은 프로젝트의 시간적 진행 단계를 나타낸다. Stage 값이 작을수록 먼저 수행되어야 하는 단계를 의미한다. 예를 들어, S1으로 시작하는 모든 Task는 S2로 시작하는 Task보다 절차적으로 선행되며, S2 Task는 S3 Task보다 선행된다(S1 → S2 → S3 → S4 → S5 순서). 이러한 절차적 순서는 SAL ID를 파싱하여 Stage 값만 추출하면 즉시 파악할 수 있으며, 별도의 시간표(Timeline) 정보나 순서 정의 없이도 자동으로 실행 순서가 결정된다.

(2) 실행적 선후 관계의 의존성 (Execution Dependency)
동일한 Stage와 Area 조합 내에서 Level 값은 작업의 실행 순서를 결정한다. Level 값이 작은 Task가 선행 작업이 되고, Level 값이 큰 Task가 후행 작업이 된다. 예를 들어, S2BA1은 S2BA2보다 먼저 실행되어야 하며, S2BA2는 S2BA1이 완료되어야 실행 가능하다(S2BA1 → S2BA2 → S2BA3 순서). 이는 dependencies 필드를 명시적으로 선언하지 않아도, Level 값의 크고 작음만으로 선후 관계가 자동으로 정의됨을 의미한다. 파서(40)는 SAL ID를 파싱하여 Stage, Area, Level 값을 추출하고, 동일 Stage-Area 내에서 Level 값을 비교하여 실행 순서를 자동으로 결정한다.

(3) 병렬성 (Parallelism)
동일한 Stage, Area, Level 조합에 Variant(a, b, c 등)가 부여된 경우, 이들은 병렬로 실행 가능한 작업임을 나타낸다. 예를 들어, S2BA1a와 S2BA1b는 동일한 Stage 2, 동일한 Backend API 영역, 동일한 Level 1 위치의 작업이나, Variant 'a'와 'b'로 구분되므로 서로 독립적으로 병렬 실행 가능하다(S2BA1a || S2BA1b). Variant가 없는 SAL ID(예: S2BA1)는 해당 좌표에 단일 작업만 존재함을 의미한다. 이러한 병렬성 정보는 SAL ID를 파싱하여 Variant 유무를 확인하는 것만으로 즉시 파악할 수 있다.

(4) 인접성 (Adjacency)
동일한 Stage와 Area 조합을 갖는 Task들은 기능적으로 관련된 작업임을 나타낸다. 예를 들어, S2BA1, S2BA2, S2BA3은 모두 Stage 2(개발 1차) 단계의 BA(Backend API) 영역에 속하므로, 백엔드 API 구현이라는 공통 맥락을 공유한다. 이들은 Level 값으로만 구분되며, 서로 인접한 기능을 수행한다. 사용자는 SAL ID의 Stage-Area 조합만 확인하면 해당 작업이 어떤 기능적 영역(Context)에 속하는지 즉시 파악할 수 있다.

이러한 구조적 특성은 SAL ID가 단순한 참조 번호가 아니라, 식별자 자체가 프로젝트의 구조 정보(절차적 순서, 실행 의존성, 병렬성, 기능적 인접성)를 인코딩한 '자기 설명적(Self-Descriptive)' 식별자임을 의미한다. 이를 통해 사용자는 SAL ID만 보고도 해당 Task의 위치, 맥락, 의존 관계를 즉시 이해할 수 있으며, 시스템은 파싱 알고리즘을 통해 이러한 정보를 자동으로 추출하여 실행 계획을 구축할 수 있다.
...
4.2 파싱 알고리즘 상세
본 발명에서 SAL ID를 파싱하는 방법은 다양하게 구현될 수 있으며, 본 명세서에서는 정규식을 활용한 파싱 방법을 일 실시예로서 설명한다. 파싱 프로세스는 다음의 순차적인 검증 단계를 거친다.
...
시퀀서는 다음의 순차적 절차를 수행한다. 첫째, 전체 Task 지시서들의 dependencies 필드를 파싱하여 각 Task에 대한 선행 Task 목록과 후행 Task 목록을 모두 추출한다. 선행 Task는 해당 Task의 dependencies 필드에 명시된 Task이며, 후행 Task는 dependencies 필드에 해당 Task를 참조하고 있는 Task이다. 둘째, 각 Task에 대해 선행-후행 의존성 관계를 분석하고, Level 값의 크고 작음이 의존성 관계와 일치하는지 검사한다. 후행 Task의 Level 값이 선행 Task의 Level 값보다 작은 경우, 의존성 관계와 Level 순서가 불일치하므로 이를 Level 충돌로 판정한다. 셋째, 충돌이 감지된 경우 해당 Task를 원하는 Level로 이동시키되, 이동 대상 Level 이상의 기존 Task들의 Level 값을 1씩 자동 증가시키거나, 이동 후 비어있는 Level 구간의 기존 Task들을 1씩 앞으로 당겨 Level 값을 자동 감소시킨다. 예를 들어 S2F9를 S2F1로 이동하는 경우, 기존 S2F1, S2F2, S2F3...을 S2F2, S2F3, S2F4...로 밀어내고, S2F6을 S2F9로 이동하는 경우 기존 S2F7, S2F8, S2F9...를 S2F6, S2F7, S2F8...로 당긴다.

5.4 배분기: 자원 배분 (54)
배분기(54)는 Task 지시서를 분석하여 작업 수행에 필요한 에이전트와 도구를 자동으로 식별하고 배정하는 모듈이다.

배분기는 다음의 배정 절차를 수행한다. 첫째, Task 지시서의 Area 코드를 확인하여 Area별 에이전트 매핑 테이블을 조회한다. 둘째, Task 지시서의 execution_type 필드를 확인한다. execution_type이 AI-Only인 경우 AI 에이전트만 배정하고, Human-AI인 경우 사람과 AI 에이전트를 모두 배정하며, Manual인 경우 사람만 배정한다. 셋째, Task 수행을 담당하는 Task 에이전트와 결과를 검증하는 검증 에이전트를 분리하여 배정한다. 동일한 에이전트가 작성과 검증을 동시에 담당하지 않도록 한다. 넷째, Task 지시서의 tools 필드에 명시된 도구 목록을 확인하여 에이전트에게 제공할 도구를 배정한다.

배분기의 에이전트 배정 규칙은 다음과 같다. Task 에이전트 매핑은 Area별로 정의된다. M(Documentation)은 documentation-specialist를, U(Design)는 frontend-developer를, F(Frontend)는 frontend-developer를, BI(Backend Infra)는 backend-developer 또는 devops-troubleshooter를, BA(Backend APIs)는 backend-developer를, D(Database)는 database-specialist를, S(Security)는 security-specialist를, T(Testing)은 test-engineer를, O(DevOps)는 devops-troubleshooter를, E(External)은 backend-developer 또는 devops-troubleshooter를, C(Content)는 content-specialist를 Task 에이전트로 배정한다.

검증 에이전트 분리 규칙은 Task 에이전트와 검증 에이전트가 반드시 다른 역할군으로 배정되도록 한다. 작성자와 검증자가 동일하면 검증의 객관성이 훼손되므로, 다음 규칙을 적용한다. Task 에이전트가 개발자 계열(frontend-developer, backend-developer, devops-troubleshooter)인 경우, 검증 에이전트는 code-reviewer 또는 qa-specialist로 배정한다. Task 에이전트가 전문가 계열(database-specialist, security-specialist, content-specialist)인 경우, 검증 에이전트는 해당 분야의 검증 전문가로 배정한다. 예를 들어 database-specialist의 경우 database-specialist를, security-specialist의 경우 security-auditor를 검증 에이전트로 배정한다. Task 에이전트가 test-engineer인 경우, 검증 에이전트는 qa-specialist로 배정한다.
...
7.1 SAL ID 체인 목록 (71)
Task의 SAL ID가 변경될 때마다, 블록체인과 유사한 방식으로 기존 ID를 삭제하지 않고 언더바(_)를 구분자로 하여 새로운 ID를 뒤에 덧붙인다. 예를 들어, S2BA1이 S2BA2로 변경되면 S2BA1_S2BA2가 되고, 다시 S2BA3으로 변경되면 S2BA1_S2BA2_S2BA3이 된다. 언더바로 구분된 마지막 세그먼트(S2BA3)가 해당 Task의 최신 SAL ID(72)이다.

7.1.1 실시예: SSAL Works 프로젝트의 SAL ID 변경 이력
SSAL Works 프로젝트에서는 프로젝트 진행 중 SAL ID 변경이 발생했다.

Sentry 모니터링 설정 Task는 초기에 S4BI1로 계획되었다. 이는 Stage 4(개발 3차)의 BI(백엔드 기반) 영역에서 모니터링을 설정하려는 계획이었다. 그러나 프로젝트 진행 중 오류 추적 시스템은 개발 초기부터 설정되어야 모든 Stage에서 발생하는 오류를 효과적으로 추적할 수 있다고 판단되었다. 따라서 이 Task는 S1BI2로 변경되어 Stage 1(개발 준비) 단계로 이동했다. 이때 SAL ID는 블록체인 방식으로 S4BI1_S1BI2로 기록된다.

이러한 변경 이력은 ID 자체에 블록체인 방식으로 인코딩되며(S4BI1_S1BI2), 언더바로 구분된 마지막 세그먼트(S1BI2)가 해당 Task의 최신 SAL ID(72)로 관리된다.

7.2 체인 덧붙이기 (73)
SAL ID가 변경될 때 기존 ID를 삭제하지 않고 블록체인 방식으로 언더바(_) 구분자를 통해 새로운 ID를 뒤에 덧붙인다(Append). Append-Only 방식으로 동작한다.

앞서 실시예에서 Sentry 모니터링 Task가 S4BI1에서 S1BI2로 변경될 때, 시스템은 기존의 S4BI1 ID를 삭제하지 않는다. 대신 언더바를 구분자로 하여 S1BI2를 덧붙여 S4BI1_S1BI2 형태로 기록한다. 이는 변경 이력의 추적성을 보장하고, 이전 ID로 참조하던 외부 시스템이나 문서가 깨지지 않도록 한다.

7.3 리졸버(74) 및 최신 ID 반환
리졸버(74)는 외부 시스템이나 사용자가 구(Old) SAL ID로 작업을 조회할 때, ID를 언더바(_)로 분리하여 가장 마지막 세그먼트를 최신 SAL ID(72)로 반환하는 기능을 수행한다. 이를 통해 SAL ID가 변경되더라도 기존 참조가 깨지지 않고 자동으로 최신 ID로 연결된다.

실시예에서 사용자가 S4BI1로 Task를 조회하면, 리졸버(74)는 해당 Task의 현재 SAL ID가 S4BI1_S1BI2임을 확인하고, 언더바로 분리하여 마지막 세그먼트인 S1BI2를 최신 SAL ID(72)로 반환한다. 사용자는 자동으로 S1BI2 Task의 정보를 받게 되며, SAL ID가 변경되었다는 사실을 인지하지 못한 채 정상적으로 작업을 조회할 수 있다.
...
(a) 컴퓨터 시스템이, Stage와 Area로 구성된 매트릭스 상에서 복수의 Task들을 선정하고 각 Task에 Level 및 Variant를 부여하여 인코딩한 SAL ID를 수신하되, 상기 SAL ID는 식별자의 형식 자체에 Stage 값에 의한 절차적 순서, 동일 Stage-Area 내에서 Level 값의 크고 작음에 의한 실행적 선후 관계의 의존성, 동일 Stage-Area-Level에서 Variant 값에 의한 병렬성, 동일 Stage-Area 조합에 의한 기능적 인접성 정보를 인코딩하여 구조적으로 내재하며, 파싱 알고리즘에 기초하여 상기 SAL ID를 파싱하고 Stage, Area, Level, Variant 구성요소를 추출하여 정규화함으로써 명시적 의존성 선언 코드 없이 의존성 구조를 자동 추론하되, 유효하지 않은 SAL ID는 시스템 흐름을 중단하지 않고 별도로 관리하는 단계;
...
(f) 상기 컴퓨터 시스템이, SAL ID가 변경되면 기존 SAL ID를 삭제하지 않고 블록체인 방식으로 언더바(_) 구분자를 통해 새로운 SAL ID를 기존 ID 뒤에 연결하여 체인 형태로 관리하되(예: S2BA1 → S2BA1_S2BA2 → S2BA1_S2BA2_S2BA3), 구(Old) SAL ID로 조회 요청이 들어오면 언더바로 분리하여 가장 마지막 세그먼트를 최신 SAL ID로 반환하는 단계;
를 포함하는 방법.

【청구항 2】
SAL ID 기반 멀티태스크 작업 순서 자동 조정 방법에 있어서,
(a) 컴퓨터 시스템이, 복수의 Task 지시서들로부터 각 Task의 dependencies 필드를 파싱하여, 각 Task에 대한 선행 Task 목록과 후행 Task 목록을 추출하되, 선행 Task는 해당 Task의 dependencies 필드에 명시된 Task이며, 후행 Task는 dependencies 필드에 해당 Task를 참조하고 있는 Task인 단계;
(b) 상기 컴퓨터 시스템이, 각 Task에 대해 선행-후행 의존성 관계를 분석하고, SAL ID에 인코딩된 Level 값의 크고 작음이 의존성 관계와 일치하는지 검사하되, 후행 Task의 Level 값이 선행 Task의 Level 값보다 작은 경우, 의존성 관계와 Level 순서가 불일치하므로 이를 Level 충돌로 판정하는 단계; 및
(c) 상기 컴퓨터 시스템이, Level 충돌이 감지된 경우 해당 Task를 원하는 Level로 이동시키되, 동일 Stage-Area 내에서 이동 대상 Level 이상의 기존 Task들의 Level 값을 1씩 자동 증가시키거나, 이동 후 비어있는 Level 구간의 기존 Task들의 Level 값을 1씩 자동 감소시켜 재조정하며, 조정된 SAL ID는 동일한 Stage와 Area 내에서 고유성을 유지하도록 하는 단계;
를 포함하는 방법.
...
Task 내용을 분석하여 실행 순서의 문제를 파악하고 순서를 조정하되, 이동 대상 Level 이상의 기존 Task들의 Level 값을 1씩 증가시키거나 이동 후 비어있는 Level 구간의 기존 Task들의 Level 값을 1씩 감소시켜 SAL ID를 조정하고 변경된 SAL ID를 dependency 필드에서 참조하는 Task들의 의존성 정보를 자동으로 갱신하는 시퀀서 모듈;
...
【청구항 9】
청구항 1에 있어서,
상기 방법은, SAL 그리드 뷰어를 통해 프로젝트 진행 현황을 Stage를 X축, Area를 Y축, Level을 Z축으로 매핑한 3차원 블록 뷰로 시각화하는 단계를 더 포함하되, 각 블록은 해당 Task의 상태에 따라 색상으로 구분되어 표시되며, 동일한 Stage-Area 셀 내에서 Level 값이 증가함에 따라 Z축 방향으로 적층되어 의존성 체인을 시각적으로 표현하는 것을 특징으로 하는 방법.
...
【청구항 14】
청구항 1에 있어서,
상기 (b) 단계의 시퀀서는, 전체 Task 지시서들의 dependencies 필드를 파싱하여 각 Task에 대한 선행 Task 목록과 후행 Task 목록을 추출하고, Level 충돌을 검출하며, 해당 Task를 원하는 Level로 이동시키되 이동 대상 Level 이상의 기존 Task들의 Level 값을 1씩 자동 증가시키거나 이동 후 비어있는 Level 구간의 기존 Task들의 Level 값을 1씩 자동 감소시켜 SAL ID를 조정하는 것을 특징으로 하는 방법.
...
【청구항 17】
청구항 1에 있어서,
상기 방법은 (f) 단계의 ID 체인 관리 없이도 SAL ID 파싱, SAL 그리드 구축, Task 실행, 검증 수행, Stage 검증을 수행할 수 있는 것을 특징으로 하는 방법.
