...
## 11. 공개 전환 작업 프로세스 ⭐ (신규 - 2025-12-24)

### 개요

현재 SSAL_Works_Private 디렉토리를 그대로 공개할 수 없음.
단계별로 정리 작업을 거쳐 공개용/패키지용으로 분리해야 함.
...
│ 1단계: 사용자(PO) 작업                                       │
├─────────────────────────────────────────────────────────────┤
│  1. SSAL_Works_Private 전체 복사                             │
│  2. Google Drive에 업로드                                    │
│  3. 이름 변경: SSAL_Works_Public                             │
│  4. 명확하게 불필요한 것 제외 (학습용 콘텐츠 등)              │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│ 2단계: AI 작업 (공개용 정리)                                 │
├─────────────────────────────────────────────────────────────┤
│  SSAL_Works_Public 디렉토리 분석 후 정리 제안:               │
│                                                              │
│  🔴 삭제 (보안)                                              │
│     - .env 파일들                                            │
│     - API Keys, 비밀번호                                     │
...
│                                                              │
│  🟠 삭제 (불필요)                                            │
│     - 학습용 콘텐츠 (부수적_고유기능/학습용_콘텐츠)           │
│     - 작업 결과물 (Orders/, Reports/, task-results/)         │
│     - 임시 파일, 로그 파일                                   │
│     - SSALWorks 전용 기능                                    │
│                                                              │
│  🟢 유지 (공개용)                                            │
│     - 표준 디렉토리 구조 (P0~S5, Production)                 │
│     - 개발 도구 (Bridge, Monitor)                            │
│     - 템플릿, 안내문                                         │
│     - 설정 파일 (.claude 등)                                 │
│                                                              │
│  → 결과: Google Drive에서 공유 가능한 상태                   │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│ 3단계: AI 작업 (설치 패키지 추출)                            │
├─────────────────────────────────────────────────────────────┤
│  공개용에서 초기 설치에 필요한 것만 추출:                     │
│                                                              │
│  🔵 패키지 포함                                              │
│     - 빈 폴더 구조 (P0~S5)                                   │
│     - 개발 도구 (Bridge, Monitor)                            │
│     - 기본 설정 파일                                         │
│     - 템플릿 (Order Sheet, Briefing)                         │
│     - SAL Grid 기본 파일                                     │
│                                                              │
│  ⚪ 패키지 제외 (공개용에는 있지만 패키지에는 미포함)         │
│     - 작업 예시/샘플                                         │
│     - 상세 문서                                              │
│     - 기존 작업 결과물                                       │
│                                                              │
...
| 분류 | 설명 | 처리 |
|------|------|------|
| 🔴 삭제 (보안) | 절대 공개 불가 | 즉시 삭제 |
| 🟠 삭제 (불필요) | 공개할 필요 없음 | 공개용에서 제외 |
| 🟢 유지 (공개용) | Google Drive 공유용 | 그대로 유지 |
| 🔵 패키지용 | 초기 설치에 포함 | 패키지에 포함 |
| ⚪ 공개용만 | 공개용에는 있지만 패키지에는 미포함 | 참고용으로 유지 |

### 현재 상태

- [ ] **1단계**: PO가 Google Drive에 SSAL_Works_Public 생성 중
- [ ] **2단계**: AI가 분석 후 삭제/유지 목록 제안 (대기)
- [ ] **3단계**: 패키지 추출 (대기)

### 참고

- 패키지 생성 스크립트: `공개_전환_업무/05_패키지_생성_스크립트.js` (초안)
- 패키지 표준 구조: `공개_전환_업무/04_패키지_표준_디렉토리_구조.md`
